...
## 2. できること
1. 補助金の検索
   - キーワード（2文字以上）、並び順、昇順/降順、受付状況は必須項目です。
   - キーワード、地域条件で検索できます（地域は候補サジェストから複数追加可能）。
   - 「全国案件を含める」で、選択地域に加えて全国対象の案件も同時に検索できます。
2. 補助金の詳細確認
   - 一覧から選択して詳細情報を表示します。
3. お気に入り管理
   - 保存、一覧表示、削除ができます。
...
5. APIトークン管理
   - トークン保存、読込、削除ができます。
6. 操作ログ
   - 過去の操作やAPIエラーを確認できます（更新/クリア可能）。

## 2.1 トークン要否
- トークン任意: 補助金検索、詳細取得、API由来の地域候補更新（未設定でも実行し、401時に設定を促します）
- トークン不要: お気に入り一覧/削除、CSV出力、地域選択UIの編集
